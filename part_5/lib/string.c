...
// 这些是函数的实现。Linus首先在string.h内写了函数头，然后在此处用汇编写了函数体。
// 这个文件会被编译为.o，在链接时发挥作用。

// 为什么不直接使用C语言，而是选择了汇编呢？总体来看，原因可能有如下几个：
// 当时的硬件性能薄弱，Linus不得不使用汇编来加速代码处理。
// C语言工具链有一些BUG，使得Linus不完全信任C语言工具链，不想在内核级别使用C语言工具链。
// Linus个人的技术志向。
