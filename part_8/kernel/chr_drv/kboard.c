...
#include <asm/io.h>
/*
    这个文件保存着所有键盘操作，包括大小写，转义字符等。
    原本这个文件是Linus用汇编写的，我们为了方便，使用C语言来做。

    只介绍基本思路，不详细介绍代码：

    键盘只会输出整数。至于这个整数表示什么字符，则完全是由软件决定的。
    所以，键盘处理流程中，首先会有一个整数数组，其次有一些字符数组。
    把整数数组的元素值作为下标，去字符数组内寻找对应的字符。

    为了解耦，此处并没有设计“键盘输入缓冲区”。键盘缓冲区在tty_io里面。
*/
